#Problematic MS-IDs (The bugs shown by these MS-IDs may have aalready been addressed):
...
tab_names = ["New Manuscript Processing"] #add more items in this list to increase amount of tabs in the GUI
...
###################################################################################################################
####                                      FOR THE IAS IT TEAM:                                                #####
####             The parsing_values[0][:] variable holds the starting/ending point strings that               #####
####         the code looks for in the text copied from the ScholarOne website.  If the ScholarOne            #####
####          website makes modifications to the way that they format data on their website, then             ##### 
####                        this variable can be modified to address those changes.                           ##### 
###################################################################################################################
parsing_values[0][:] = "JIAS-2020", "Submitted: ", "Title:", " (proxy) (contact)", "Wiley - Manuscript type:", \
"previous submission:", "Submitting Author:", "Running Head:", "Author's Cover Letter:",                       \
"If you have been invited to submit an article for a supplement, please select the title of the supplement:",  \
 "Discipline:", "Overall Similarity Index Percentage:"
###################################################################################################################
...
def intersperse(lst, item):
    """ For cleaning up the cover letter document that is created """
...
def globalClipboardPasta():
    """ Cheap/easy way to get access to the self.variable for the tkinter button command in the GUI """ 
    pyperclip.copy(global_copy_pasta)

def clear_paragraph(self, paragraph):
    """ For creating the MS Details doc """
...
#get MS Authors
                    ...
                    tmp_all_au = (line.lower()).title() #if authors submit names as lower() or upper(), this fixes it
...
#Get MS Ithenticate Score (needs post-processing)
    ...
    def CopyExcelFormatToClipboard(self):
        """ create the customized text to send to the clipboard """
    ...
    def CreateFolderForManuscript(self):
        """ Creates the manuscript folder in the user's "Downloads" directory """
        global ms_folder
        try:
    ...
    def CreateCoverLetterAndPlaceInFolder(self):
        """ Creates the Cover Letter and places it in the folder in the user's "Downloads" directory """
        try:
            cover_letter_to_doc = ""
    ...
    def CreateMSDetailsAndPlaceInFolder(self):
        """ Creates the MSDetails document and places it in the folder in the user's "Downloads" directory """
        """ If modifications are desired to the format of the MSDetails document, then this function can be updated  to incorporate them """
        try:
            str_all_co = ""
            ...
            #this variable can be updated if modifications are wanted for the MSDetails doc
    ...
    def PrintParsingResults(self):
        """ Prints the data parsed from the ScholarOne text """
        try:
            print('ID:\t\t\t\t\t', self.ms_id)
    ...
    def FindFilesInDownloadFolder(self, *args):
        """ Searches the user's "Downloads" directory for manuscript files"""
        """ the *args argument allows for files that you'd like to ignore within the download folder """
...
def Parser():
    """ Function created to let the Tkinter GUI call for a parsing object """
    ...
    #updates gui text boxes with MS data
...
def RenameFilesAndAddToMsFolder():
    """ Renames files in the "Download" directory and moves them to the folder """
...
def get_editorial_folder():
    """ get user's JIAS editorial directory """
...
def get_MsLog_folder():
    """ get the directory in which the user has the MSLog file """
...
def slice_folder_name(my_str, sub):
    """ Helper function for processing folder names """
...
def prepend_multiple_lines(file_name, list_of_lines):
    """ for generating the ScholarOne/SharePoint MS check .txt file format """
...
def walklevel(some_dir, level=1):
    """ for finding files for the ScholarOne/SharePoint MS check tool """
...
def updateMsLog():
    """ large function that updates the MSLog based on step-by-step protocol from the Intern's Manual """ 
...
def check_for_s1_ms_in_editorial_folders():
    """ runs the S1/SP check tool via Tkinter button """
    global missing_ms
...
def MsLogUpdate_popup():
    """ generate a pop-up window for the MsLog Updater tool """
    global msLog_window
...
def S1_check_popup():
    """ generate a pop-up window for the S1/SP Manuscript checker tool """
    global s1_window
    #global results_label_text
    #results_label_text = StringVar()
...
class Main:
    """ class for updating the app to newer versions """
